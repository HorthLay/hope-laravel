...
class ChildMediaController extends Controller
{
 public function store(Request $request, SponsoredChild $child)
    {
        ...
        $folder = public_path("uploads/children/{$child->id}/media");
        ...
        $child->media()->create([
            'type'       => $type,
            'file_path'  => "uploads/children/{$child->id}/media/{$filename}",
    ...
    public function destroy(SponsoredChild $child, ChildMedia $media)
    {
        abort_if($media->child_id !== $child->id, 403);
