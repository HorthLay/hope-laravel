...
<head>
    ...
    <meta name="description" content="{{ $category->description ?? 'Browse all articles in '.$category->name.' on '.($settings['site_name'] ?? 'Hope & Impact').'.' }}">
...
<div class="relative overflow-hidden py-14 md:py-20"
    ...
    <div class="max-w-6xl mx-auto px-4 relative text-white text-center">
        ...
        <div class="flex items-center justify-center gap-3 text-sm text-white/80">
            ...
            <span>·</span>
            <a href="{{ route('home') }}" class="hover:text-white transition"><i class="fas fa-home mr-1"></i>Home</a>
            <span>·</span>
...
<div class="bg-white border-b border-gray-100 shadow-sm sticky top-0 z-40">
    <div class="max-w-6xl mx-auto px-4 py-3">
        <div class="flex items-center gap-2 overflow-x-auto scrollbar-hide">
            <a href="{{ route('home') }}" class="flex-shrink-0 px-4 py-2 rounded-full text-xs font-bold bg-gray-100 hover:bg-gray-200 text-gray-600 transition">
...
<div class="max-w-6xl mx-auto px-4 py-10 md:py-14">
    ...
    <div class="relative group overflow-hidden rounded-2xl shadow-xl mb-8 scroll-animate">
        ...
        <div class="w-full h-72 md:h-96 flex items-center justify-center"
            ...
            <i class="{{ $category->icon ?? 'fas fa-newspaper' }} text-8xl opacity-20" style="color:{{ $category->color ?? '#f97316' }}"></i>
        </div>
        @endif
        ...
        <div class="absolute inset-0 flex flex-col justify-end p-6 md:p-10">
            ...
            <h2 class="text-2xl md:text-4xl font-black text-white mb-3 leading-tight max-w-3xl">{{ $first->title }}</h2>
            @if($first->excerpt)
            <p class="text-white/80 text-sm md:text-base mb-4 line-clamp-2 max-w-2xl hidden md:block">{{ Str::limit(strip_tags($first->excerpt), 150) }}</p>
            @endif
        ...
        <div class="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 group scroll-animate flex flex-col">
            <div class="overflow-hidden flex-shrink-0">
                ...
                <div class="w-full h-48 flex items-center justify-center"
                    ...
                    <i class="{{ $category->icon ?? 'fas fa-newspaper' }} text-5xl opacity-20" style="color:{{ $category->color ?? '#f97316' }}"></i>
                </div>
                @endif
            </div>
            <div class="p-5 flex flex-col flex-1">
                ...
                <h3 class="text-base font-bold text-gray-900 mb-2 line-clamp-2 group-hover:text-orange-500 transition leading-snug flex-1">{{ $article->title }}</h3>
                @if($article->excerpt)
                <p class="text-xs text-gray-500 line-clamp-2 mb-3">{{ Str::limit(strip_tags($article->excerpt), 90) }}</p>
                @endif
    ...
    <div class="flex items-center justify-center gap-2 mt-8 flex-wrap">
        ...
        <span class="px-4 py-2 rounded-full text-sm bg-gray-100 text-gray-400 cursor-not-allowed"><i class="fas fa-arrow-left mr-1"></i> Prev</span>
        @else
        <a href="{{ $articles->previousPageUrl() }}" class="px-4 py-2 rounded-full text-sm font-semibold bg-white border-2 text-gray-700 hover:border-orange-400 hover:text-orange-500 transition shadow-sm"><i class="fas fa-arrow-left mr-1"></i> Prev</a>
        @endif
        @foreach($articles->getUrlRange(max(1, $articles->currentPage()-2), min($articles->lastPage(), $articles->currentPage()+2)) as $page => $url)
            @if($page == $articles->currentPage())
            <span class="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold text-white shadow" style="background-color:{{ $category->color ?? '#f97316' }}">{{ $page }}</span>
            @else
            <a href="{{ $url }}" class="w-10 h-10 rounded-full flex items-center justify-center text-sm font-semibold bg-white border-2 border-gray-200 text-gray-700 hover:border-orange-400 hover:text-orange-500 transition shadow-sm">{{ $page }}</a>
            @endif
        @endforeach
        @if($articles->hasMorePages())
        <a href="{{ $articles->nextPageUrl() }}" class="px-4 py-2 rounded-full text-sm font-semibold bg-white border-2 text-gray-700 hover:border-orange-400 hover:text-orange-500 transition shadow-sm">Next <i class="fas fa-arrow-right ml-1"></i></a>
        @else
        <span class="px-4 py-2 rounded-full text-sm bg-gray-100 text-gray-400 cursor-not-allowed">Next <i class="fas fa-arrow-right ml-1"></i></span>
        @endif
    </div>
    <p class="text-center text-xs text-gray-400 mt-3">Showing {{ $articles->firstItem() }}–{{ $articles->lastItem() }} of {{ $articles->total() }} articles</p>
    @endif

    @else
    <div class="text-center py-20">
        <div class="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-5" style="background-color:{{ $category->color ?? '#f97316' }}15;">
            <i class="{{ $category->icon ?? 'fas fa-newspaper' }} text-4xl opacity-40" style="color:{{ $category->color ?? '#f97316' }}"></i>
        </div>
        ...
        <a href="{{ route('home') }}" class="inline-flex items-center gap-2 px-6 py-3 rounded-full font-bold text-sm text-white transition hover:opacity-90" style="background-color:{{ $category->color ?? '#f97316' }}">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
            ...
            <a href="{{ route('category.articles', $cat->encrypted_slug) }}"
               ...
               onmouseout="this.style.borderColor='transparent';">
                @if($cat->icon)
                <div class="w-11 h-11 rounded-full flex items-center justify-center transition group-hover:scale-110" style="background-color:{{ $cat->color ?? '#f97316' }}22;">
