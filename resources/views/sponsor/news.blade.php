...
{{-- ════════════════════ PAGE BODY ════════════════════ --}}
<div class="pw" style="max-width: 1080px; margin: 0 auto; padding-top: 48px;">
    
    {{-- ══════════ HERO SECTION ══════════ --}}
    <div style="background: linear-gradient(135deg, #2a3328 0%, #1a2019 100%); border-radius: 24px; padding: 48px; display: flex; align-items: center; justify-content: space-between; gap: 32px; margin-bottom: 48px; box-shadow: 0 20px 40px rgba(0,0,0,.15); position: relative; overflow: hidden;" class="anim-up">
        <div style="position: absolute; right: -50px; top: -100px; width: 300px; height: 300px; background: radial-gradient(circle, rgba(239,125,0,0.15), transparent 70%); border-radius: 50%;"></div>
        <div style="position: absolute; left: 20%; bottom: -150px; width: 250px; height: 250px; background: radial-gradient(circle, rgba(239,125,0,0.1), transparent 70%); border-radius: 50%;"></div>
        
        <div style="position: relative; z-index: 1;">
            <div style="display: inline-flex; align-items: center; justify-content: center; width: 56px; height: 56px; background: rgba(239,125,0,0.15); border: 1px solid rgba(239,125,0,0.25); border-radius: 16px; margin-bottom: 20px;">
                <i class="far fa-newspaper" style="color: var(--brand); font-size: 24px;"></i>
            </div>
            <h1 style="color: #fff; font-family: 'Lora', serif; font-size: 40px; font-weight: 700; margin-bottom: 16px; line-height: 1.15;">Organization News & Updates</h1>
            <p style="color: rgba(255,255,255,0.7); font-size: 16px; max-width: 540px; line-height: 1.6;">Stay informed about the latest activities, field reports, and inspiring stories from the ground. Your support makes these moments possible.</p>
        </div>
        
        <div style="position: relative; z-index: 1; flex-shrink: 0;" class="hidden md:block">
            <div style="width: 200px; height: 200px; border-radius: 50%; border: 2px dashed rgba(255,255,255,0.1); display: flex; align-items: center; justify-content: center; position: relative;">
                <div style="position: absolute; inset: -10px; border: 1px solid rgba(239,125,0,0.3); border-radius: 50%; animation: spin 20s linear infinite;"></div>
                <div style="width: 140px; height: 140px; background: linear-gradient(135deg, rgba(255,255,255,0.05), rgba(255,255,255,0)); border-radius: 50%; backdrop-filter: blur(4px); display: flex; align-items: center; justify-content: center; box-shadow: inset 0 0 20px rgba(255,255,255,0.02);">
                    <i class="fas fa-bullhorn" style="font-size: 56px; color: var(--brand); opacity: 0.9;"></i>
                </div>
            </div>
        </div>
    </div>
    ...
    {{-- ══════════ NEWS GRID ══════════ --}}
    <div style="margin-bottom: 40px;" class="anim-up" style="animation-delay: 0.1s;">
        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 32px;">
            <h2 style="font-family: 'Lora', serif; font-size: 28px; font-weight: 700; color: var(--dark);">Latest Articles</h2>
            <div style="height: 1px; flex: 1; background: var(--border); margin: 0 24px;"></div>
            <span style="color: var(--muted); font-size: 13px; font-weight: 700; background: var(--brand-lt); padding: 4px 12px; border-radius: 999px;">{{ $articles->total() }} Articles</span>
        </div>

        @if($articles->count() > 0)
            <div class="c-grid" style="grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 32px;">
                @foreach($articles as $index => $article)
                    @php 
                        $isFeatured = $index === 0 && $articles->currentPage() === 1;
                    @endphp
                    <a href="{{ route('articles.show', $article->encrypted_slug) }}" 
                       class="bg-white rounded-[24px] border border-[#ede9e3] shadow-[0_4px_24px_rgba(0,0,0,.04)] overflow-hidden group transition-all duration-400 hover:-translate-y-2 hover:shadow-[0_12px_40px_rgba(239,125,0,.12)] block flex flex-col {{ $isFeatured ? 'md:col-span-2 md:flex-row' : '' }}" 
                       style="text-decoration:none;">
                        
                        <div class="relative overflow-hidden {{ $isFeatured ? 'md:w-[50%] md:h-auto h-64' : 'w-full h-56' }}">
                            @if($article->image)
                                <img src="{{ $article->image->url }}" alt="{{ $article->title }}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105">
                            @else
                                <div class="w-full h-full bg-[#f8f7f3] flex items-center justify-center"><i class="far fa-image text-5xl text-[#d6d3cd]"></i></div>
                            @endif
                            <div class="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                            
                            @if($isFeatured)
                                <div class="absolute top-5 left-5 bg-[#ef7d00] text-white text-[10px] font-black uppercase tracking-wider py-1.5 px-3 rounded-md shadow-lg flex items-center gap-1.5">
                                    <i class="fas fa-star text-[9px]"></i> Featured
                                </div>
                            @endif
                        </div>
                        
                        <div class="p-8 flex flex-col flex-1 {{ $isFeatured ? 'md:w-[50%] md:justify-center' : '' }}">
                            <div class="flex items-center gap-3 mb-4">
                                <span class="text-[11px] font-bold text-[#ef7d00] uppercase tracking-wider bg-[#fff4db] py-1.5 px-3 rounded-md">Update</span>
                                <span class="text-[12px] font-semibold text-[#a3978c]"><i class="far fa-calendar-alt mr-1.5"></i>{{ $article->published_at ? $article->published_at->format('M d, Y') : '' }}</span>
                            </div>
                            
                            <h3 class="font-bold text-[#2a3328] {{ $isFeatured ? 'text-[26px] mb-4' : 'text-[18px] mb-4' }} leading-tight group-hover:text-[#ef7d00] transition-colors" style="font-family: 'Lora', serif;">{{ $article->title }}</h3>
                            
                            @if($isFeatured)
                                <p class="text-[#645c55] text-[14px] leading-relaxed mb-6 flex-1">{{ Str::limit(strip_tags($article->content), 180) }}</p>
                            @endif
                            
                            <div class="{{ $isFeatured ? 'mt-4' : 'mt-auto' }} pt-5 border-t border-[#f0ece6] flex items-center justify-between">
                                <span class="text-[13px] font-bold text-[#2a3328] group-hover:text-[#ef7d00] transition-colors">Read Article</span>
                                <div class="w-8 h-8 rounded-full bg-[#f8f7f3] flex items-center justify-center group-hover:bg-[#ef7d00] transition-colors">
                                    <i class="fas fa-arrow-right text-[12px] text-[#a3978c] group-hover:text-white transition-colors group-hover:translate-x-0.5 transform"></i>
                                </div>
                            </div>
                        </div>
                    </a>
        ...
        @else
            <div class="wcard anim-up flex flex-col items-center justify-center py-20 text-center" style="background: linear-gradient(135deg, #fff, #fcfbfa); border: 2px dashed var(--border);">
                <div class="w-20 h-20 bg-[#fff4db] rounded-full flex items-center justify-center mb-6 shadow-inner">
                    <i class="far fa-newspaper text-3xl text-[#ef7d00]"></i>
                </div>
                <h3 class="font-bold text-[24px] text-[#2a3328] mb-3" style="font-family: 'Lora', serif;">No Articles Yet</h3>
                <p class="text-[#a3978c] text-[15px] max-w-md mx-auto">Check back later for the latest news and updates from the organization.</p>
            </div>
        @endif
    </div>
</div>
