...
<section class="page-hero" style="min-height:380px">
    ...
    <div class="page-hero-content">
        ...
        <div class="pill bg-orange-500/20 border border-orange-400/30 text-orange-300 mb-5" style="animation:fadeUp .7s ease both">
            <i class="fas fa-heart text-xs"></i> Philanthropy
        </div>
        <h1 class="text-4xl md:text-5xl font-black text-white leading-tight mb-4" style="animation:fadeUp .9s ease both">Family Foundations & Philanthropy</h1>
        <p class="text-lg text-white/80 font-medium max-w-xl" style="animation:fadeUp .9s .15s ease both">Strategic, values-driven giving that creates a lasting legacy for children in Cambodia.</p>
    </div>
</section>
...
<section class="section bg-white py-16 md:py-20">
    ...
    <div class="bg-gradient-to-br from-orange-50 to-amber-50 rounded-3xl p-10 mb-12 border border-orange-100">
        <h2 class="text-2xl font-black text-gray-900 mb-5">Why Family Foundations Choose Des Ailes pour Grandir</h2>
        <p class="text-gray-600 leading-relaxed mb-4">Family foundations and philanthropists seek impact that is traceable, values-aligned, and enduring. Our programs in Cambodia deliver all three — with rigorous reporting and a deeply human approach.</p>
        <p class="text-gray-600 leading-relaxed">Whether you wish to fund a specific program, name a project, or build a multi-year partnership, we are here to create a giving arrangement that reflects your family's values and vision.</p>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
        @foreach([
            ['fas fa-eye', 'teal',  'Full Transparency', 'Dedicated impact reports, financial breakdowns, and direct access to our field team.'],
            ['fas fa-tag', 'orange','Named Projects',    'Name a school, well, or scholarship fund after your family — a living legacy for children.'],
            ['fas fa-handshake', 'blue', 'Multi-Year Giving', 'Predictable, structured giving allows us to plan and scale programs for greater impact.'],
            ['fas fa-certificate', 'green', 'Full Fiscal Benefit', 'All contributions are eligible for the maximum applicable tax deductions in France.'],
        ] as $item)
        <div class="flex items-start gap-4 p-6 section-card ">
            <div class="w-11 h-11 bg-{{ $item[1] }}-100 rounded-xl flex items-center justify-center flex-shrink-0">
                <i class="{{ $item[0] }} text-{{ $item[1] }}-500"></i>
            </div>
            <div>
                <h3 class="font-black text-gray-800 mb-1">{{ $item[2] }}</h3>
    ...
    <div class="bg-[#1a2e3b] rounded-3xl p-10 text-white text-center">
        <h2 class="text-2xl font-black mb-3">Begin a Conversation</h2>
        <p class="text-white/70 mb-6">We welcome all exploratory conversations — confidential, with no obligation.</p>
        <a href="{{ route('home') }}#contact" class="inline-flex items-center gap-2 px-8 py-3 bg-orange-500 hover:bg-orange-600 text-white font-black rounded-xl transition">
            <i class="fas fa-envelope text-sm"></i> Contact Our Foundation Team
        </a>
    </div>
</div>
</section>
