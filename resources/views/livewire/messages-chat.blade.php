...
<div
    ...
    x-data="{
        ...
        handleFileChange(e) {
            ...
            // If image, show local preview immediately
...
>
    ...
    <div class="chat-panel">
        ...
        <div class="chat-body" id="chat-body">
                    ...
                    <div class="msg-row {{ $isMe ? 'me' : '' }}">
                        ...
                        <div class="bubble-wrap">
                            ...
                            @if(!empty($msg['attachment_url']) && $isImg)
                                <div class="bubble {{ $isMe ? 'me' : 'them' }} bubble-img-wrap">
                                    ...
                                    @if(!empty($msg['body']))
                                        <p class="img-caption">{{ $msg['body'] }}</p>
                                    @endif
                                </div>

                            {{-- ── File bubble (non-image) ── --}}
                            @elseif(!empty($msg['attachment_url']) && !$isImg)
                                <div class="bubble {{ $isMe ? 'me' : 'them' }}">
                                    @if(!empty($msg['body']))
                                        <p style="margin-bottom:8px">{{ $msg['body'] }}</p>
                                    @endif
                                    @php
                            ...
                            @if($lp)
                                <a href="{{ $lp['url'] }}" target="_blank" rel="noopener" class="link-preview {{ $isMe ? 'lp-me' : 'lp-them' }}">
                                    @if(!empty($lp['image']))
                                        <img src="{{ $lp['image'] }}" class="lp-img" alt="" loading="lazy" onerror="this.style.display='none'">
                                    @endif
                                    <div class="lp-body">
                                        <div class="lp-host">
                                            <i class="fas fa-globe" style="font-size:9px;margin-right:4px"></i>
                                            {{ $lp['host'] ?? '' }}
                                        </div>
                                        @if(!empty($lp['title']))
            ...
            {{-- Poll pulse (tiny, only shows during poll) --}}
        ...
        <div class="chat-footer">
            ...
            <p class="footer-hint">
                ...
                Private & secure · <kbd>Enter</kbd> to send · <kbd>Shift+Enter</kbd> for new line
            </p>
        </div>

    </div>{{-- /chat-panel --}}

{{-- ═══════════════════ STYLES (inside single root) ═══════════════════ --}}
<style>
...
.chat-panel {
    ...
    overflow: hidden; animation: scaleUp .35s ease both;
}

/* ── Header ── */
.chat-header {
    padding: 16px 20px; border-bottom: 1px solid var(--border);
    display: flex; align-items: center; gap: 13px; flex-shrink: 0; background: #fff;
}
.chat-header-icon {
    width: 46px; height: 46px; border-radius: 13px;
    background: linear-gradient(135deg, var(--brand-lt), #fde9b8);
    display: flex; align-items: center; justify-content: center;
    color: var(--brand); font-size: 19px;
}
...
.unread-chip {
    ...
    font-size: 10px; font-weight: 900; padding: 3px 9px; animation: fadeIn .3s ease;
}
...
.date-divider span {
    font-size: 11px; font-weight: 700; color: var(--muted); background: #fff;
...
.unread-banner {
    ...
    padding: 7px 13px; font-size: 12px; font-weight: 700; color: #92400e;
}
...
.bubble {
    ...
    font-size: 13.5px; line-height: 1.65; font-weight: 500; word-break: break-word;
}
...
.bubble.me   {
    ...
    border-bottom-right-radius: 4px; box-shadow: 0 4px 16px rgba(239,125,0,.28);
}

/* ── Image bubble ── */
.bubble-img-wrap { padding: 6px; overflow: hidden; }
.img-link {
    display: block; position: relative; border-radius: 11px; overflow: hidden;
    line-height: 0;
}
.chat-img {
...
.img-caption {
    font-size: 13px; line-height: 1.5; margin-top: 7px; font-weight: 500;
    padding: 0 4px;
}
...
.bubble-attachment {
    ...
    padding: 8px 10px; border: 1px solid rgba(255,255,255,.25); transition: opacity .18s;
}
...
.link-preview {
    ...
    border: 1.5px solid var(--border); max-width: 300px;
...
.lp-them { border-color: var(--border); }
...
.lp-me .lp-desc { color: rgba(255,255,255,.65); }
...
.chat-empty {
    ...
    color: var(--muted); padding: 48px; text-align: center; margin: auto;
}
.chat-empty-icon {
    width: 72px; height: 72px; border-radius: 20px; background: var(--brand-lt);
    display: flex; align-items: center; justify-content: center;
    font-size: 28px; color: var(--brand); animation: scaleUp .5s ease both;
}
...
.qr-chip {
    ...
    background: #f5f2ed; border: 1.5px solid var(--border);
    font-size: 11.5px; font-weight: 700; color: #5a4e43;
    cursor: pointer; font-family: inherit; transition: all .18s;
    white-space: nowrap;
}
.qr-chip i { font-size: 10px; color: var(--orange); }
.qr-chip:hover {
    background: var(--brand-lt); border-color: var(--orange);
    color: var(--dark); transform: translateY(-1px);
    box-shadow: 0 3px 10px rgba(239,125,0,.15);
}
...
.attach-img-preview {
    ...
    background: #f5f2ed; border-radius: 12px; padding: 8px 10px;
...
.attach-file-preview {
    ...
    background: #f5f2ed; border-radius: 9px; padding: 6px 10px;
...
.chat-form {
    ...
    background: #f8f7f4; border-radius: 14px;
...
.chat-form textarea {
    ...
    font-family: inherit; font-size: 13.5px; color: var(--dark);
...
.attach-btn {
    ...
    display: flex; align-items: center; justify-content: center; font-size: 13px; transition: all .18s;
}
...
.send-btn {
    ...
    display: flex; align-items: center; justify-content: center;
    font-size: 14px; transition: all .2s; box-shadow: 0 4px 12px rgba(239,125,0,.3);
}
...
.flash-success {
    ...
    padding: 7px 12px; font-size: 12px; font-weight: 700; color: #166534;
    margin-bottom: 10px; animation: fadeIn .3s ease;
}
.footer-hint {
    font-size: 10px; color: var(--muted); margin-top: 8px; font-weight: 600; padding: 0 2px;
}
.footer-hint kbd {
    background: #f0ece7; border-radius: 4px; padding: 1px 5px;
