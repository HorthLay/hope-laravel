...
<div
    ...
    x-init="
        scrollBottom();
        $wire.on('scroll-bottom',  () => { $nextTick(() => scrollBottom()); });
        $wire.on('new-messages',   () => { $nextTick(() => scrollBottom()); });
    ...
    <div class="amc-chat">
            ...
            @else
                ...
                @foreach($messages as $msg)
                    ...
                    @if($dateLabel !== $lastDate)
                        <div class="amc-date-div"><span>{{ $dateLabel }}</span></div>
                    ...
                    <div class="amc-msg-row {{ $isAdmin ? 'amc-msg-row--me' : '' }}" wire:key="msg-{{ $msg['id'] }}">
                        ...
                        @else
                            <div class="amc-msg-icon amc-msg-icon--sponsor">
                                {{ $activeSponsorInit }}
                            </div>
                        @endif
                        ...
                        <div class="amc-bubble-wrap {{ $isAdmin ? 'amc-bubble-wrap--me' : '' }}">
                            ...
                            @else
                                ...
                                @if(!empty($msg['attachment_url']) && $isImg)
                                    <div class="amc-bubble {{ $isAdmin ? 'amc-bubble--admin' : 'amc-bubble--sponsor' }} amc-bubble--img">
                                        ...
                                        @if(!empty($msg['body']))
                                            <p class="amc-img-caption">{{ $msg['body'] }}</p>
                                        @endif
                                    </div>

                                {{-- ── File bubble ── --}}
                                @elseif(!empty($msg['attachment_url']) && !$isImg)
                                    <div class="amc-bubble {{ $isAdmin ? 'amc-bubble--admin' : 'amc-bubble--sponsor' }}">
                            ...
                            <div class="amc-msg-meta {{ $isAdmin ? 'amc-msg-meta--me' : '' }}">
                                @if(!$isAdmin)<span style="font-weight:700;color:#6b7280">{{ $activeSponsorName }}</span> · @endif
                                <span>{{ $timeLabel }}</span>
                                @if($msg['is_edited'])<span class="amc-edited">(edited)</span>@endif

                                {{-- Action buttons (only outside edit mode) --}}
                                @if(!$isEditing)
        ...
        <div class="amc-footer">
            ...
            <p class="amc-footer-hint">
                ...
                Admin reply · <kbd>Enter</kbd> to send · <kbd>Shift+Enter</kbd> new line
            </p>
        </div>
        @endif
...
.amc-search-input {
    ...
    font-size: 12.5px; font-family: inherit; font-weight: 500; color: #374151;
...
/* Thread list */
.amc-thread-list {
    flex: 1; overflow-y: auto; scrollbar-width: thin; scrollbar-color: #e5e7eb transparent;
}
...
.amc-thread {
    ...
    border-bottom: 1px solid #f9fafb; cursor: pointer; transition: background .12s;
    position: relative;
}
.amc-thread:hover { background: #fff7ed; }
.amc-thread--active { background: #fff7ed; border-left: 3px solid #f97316; }
.amc-thread--unread { background: #fffbf5; }
...
.amc-thread-dot {
    ...
    width: 10px; height: 10px; background: #f97316;
...
.amc-thread-name { font-size: 12.5px; font-weight: 700; color: #111827; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 150px; }
.amc-thread-time { font-size: 10px; color: #9ca3af; font-weight: 600; flex-shrink: 0; }
.amc-thread-preview { font-size: 11.5px; color: #6b7280; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin-bottom: 4px; }
.amc-thread-meta { display: flex; align-items: center; gap: 5px; }
.amc-tag {
    font-size: 9px; font-weight: 800; padding: 2px 7px; border-radius: 999px;
    background: #fff3e0; color: #f97316; text-transform: uppercase; letter-spacing: .04em;
}
.amc-thread-empty {
    display: flex; flex-direction: column; align-items: center; justify-content: center;
    padding: 48px 24px; color: #d1d5db; text-align: center; gap: 8px;
}
...
.amc-icon-btn {
    width: 34px; height: 34px; border-radius: 9px; background: #f9fafb;
    border: 1px solid #e5e7eb; color: #9ca3af; cursor: pointer;
...
.amc-chat-empty {
    display: flex; flex-direction: column; align-items: center; justify-content: center;
    gap: 12px; color: #9ca3af; padding: 48px; text-align: center; height: 100%;
}
.amc-chat-empty-icon {
    width: 68px; height: 68px; border-radius: 18px; background: #fff7ed;
    display: flex; align-items: center; justify-content: center;
    font-size: 26px; color: #f97316;
}
...
/* Bubbles */
.amc-bubble {
    padding: 10px 13px; border-radius: 15px; font-size: 13px;
    line-height: 1.65; font-weight: 500; word-break: break-word;
}
.amc-bubble--sponsor { background: #f3f4f6; color: #111827; border-bottom-left-radius: 3px; }
.amc-bubble--admin   {
    background: linear-gradient(135deg, #f97316, #ea580c); color: #fff;
    border-bottom-right-radius: 3px; box-shadow: 0 3px 12px rgba(249,115,22,.3);
}
...
.amc-file-att {
    ...
    padding: 7px 10px; border: 1px solid rgba(255,255,255,.25); transition: opacity .15s;
}
...
.amc-link-preview {
    ...
    max-width: 280px; background: #fff; transition: box-shadow .2s, transform .2s;
}
...
/* Inline edit box */
.amc-edit-box {
    background: #fff; border: 1.5px solid #f97316; border-radius: 12px;
...
.amc-edit-textarea {
    ...
    font-weight: 500; line-height: 1.6; background: none; min-height: 40px;
}
.amc-edit-actions {
...
.amc-form {
    ...
    border: 1.5px solid #e5e7eb; padding: 9px 10px;
...
.amc-textarea {
    ...
    font-family: inherit; font-size: 13px; color: #374151;
    font-weight: 500; line-height: 1.6; max-height: 120px; min-height: 22px;
}
...
.amc-send-btn {
    ...
    display: flex; align-items: center; justify-content: center;
    font-size: 13px; transition: all .18s; box-shadow: 0 3px 10px rgba(249,115,22,.3);
}
...
/* Attach preview in footer */
.amc-attach-preview { margin-bottom: 8px; }
.amc-img-preview-row, .amc-file-preview-row {
...
.amc-modal {
    background: #fff; border-radius: 18px; padding: 28px 24px; width: 100%;
    max-width: 360px; box-shadow: 0 20px 60px rgba(0,0,0,.18); text-align: center;
}
.amc-modal-icon {
...
@media (max-width: 900px) {
    .amc-layout { grid-template-columns: 260px 1fr; }
}
