{{-- resources/views/admin/sponsors/partials/_family_picker.blade.php --}}
{{--
    Props:
      $families          — Collection of all active Family
      $selectedFamilyIds — array of already-selected IDs
--}}
@php $selectedFamilyIds = $selectedFamilyIds ?? []; @endphp
...
<div class="card" id="family-picker-card">
    ...
    <p class="text-xs text-gray-400 mb-3">Assign one or more families to this sponsor</p>

    {{-- Selected chips --}}
    <div id="family-chips" class="flex flex-wrap gap-2 mb-3 {{ count($selectedFamilyIds) ? '' : 'hidden' }}">
        @foreach($families->whereIn('id', $selectedFamilyIds) as $family)
        <div class="family-chip flex items-center gap-1.5 pl-1 pr-2 py-1 bg-purple-50 border border-purple-200 rounded-full text-xs font-semibold text-purple-700"
            ...
            <button type="button" data-remove-family="{{ $family->id }}"
                    class="family-remove-btn ml-0.5 w-4 h-4 flex items-center justify-center rounded-full hover:bg-purple-200 transition">
    ...
    <div class="relative mb-2">
        ...
        <input type="text"
               ...
               oninput="window._familyPicker.filter(this.value)"
               onfocus="window._familyPicker.openDropdown()"
               autocomplete="off"
               class="w-full pl-8 pr-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 outline-none transition">
    </div>
    ...
    <p id="family-empty-hint" class="text-xs text-gray-400 mt-2 {{ count($selectedFamilyIds) ? 'hidden' : '' }}">
        <i class="fas fa-info-circle text-purple-300 mr-1"></i>No families assigned. Search above to add.
    </p>
</div>
...
(function () {
    ...
    const _placeholder = '{{ asset('images/family-placeholder.jpg') }}';
    ...
    function add(id, name, code, photo) {
        id = parseInt(id);
        ...
        chip.dataset.id = id;
        chip.innerHTML = `
            <img src="${photo}" class="w-5 h-5 rounded-full object-cover border border-purple-300" onerror="this.src='${_placeholder}'">
            <span>${name}${code ? ' (' + code + ')' : ''}</span>
            <button type="button" data-remove-family="${id}" class="family-remove-btn ml-0.5 w-4 h-4 flex items-center justify-center rounded-full hover:bg-purple-200 transition">
                <i class="fas fa-times text-[9px] pointer-events-none"></i>
            </button>
        ...
        const box = document.querySelector(`#family-opt-${id} .family-opt-check`);
        if (box) {
    ...
    function clearAll() {
        [..._selected].forEach(id => remove(id));
    }
    ...
    // ── Event delegation ──────────────────────────────────────────────
    document.getElementById('family-picker-card').addEventListener('click', function (e) {
        // Clear all
        if (e.target.closest('#family-clear-all-btn')) {
            clearAll();
            return;
        }

        // Remove (×) chip button
        const removeBtn = e.target.closest('[data-remove-family]');
        if (removeBtn) {
            remove(removeBtn.dataset.removeFamily);
            return;
        }

        // Dropdown option row
        const option = e.target.closest('.family-option');
        if (option) {
            const id = parseInt(option.dataset.id);
            _selected.has(id) ? remove(id) : add(id, option.dataset.name, option.dataset.code, option.dataset.photo);
            return;
        }
    });

    // Expose for oninput / onfocus on the search input
    window._familyPicker = { filter, openDropdown };
